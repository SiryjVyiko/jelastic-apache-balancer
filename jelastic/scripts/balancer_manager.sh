...
function _add_common_host(){
    ...
    echo ' ' >> /etc/httpd/conf/worker.properties;
    echo 'worker.node-s'${host_num}'.type=ajp13   ###FOR HOST '${host} >> /etc/httpd/conf/worker.properties;
    echo 'worker.node-s'${host_num}'.host'=${host} >> /etc/httpd/conf/worker.properties;
    echo 'worker.node-s'${host_num}'.port=8009   ###FOR HOST '${host} >> /etc/httpd/conf/worker.properties;
}
