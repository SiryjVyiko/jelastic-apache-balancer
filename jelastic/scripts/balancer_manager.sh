...
function _add_common_host(){
    ...
    echo 'worker.server'${host_num}'.type=ajp13   ###FOR HOST '${host} >> /etc/httpd/conf/worker.properties;
    echo 'worker.server'${host_num}'.host'=${host} >> /etc/httpd/conf/worker.properties;
    echo 'worker.server'${host_num}'.port=8009   ###FOR HOST '${host} >> /etc/httpd/conf/worker.properties;
}
