...
function _enableSSL(){
        ...
        sed -i "/^#LoadModule.*ssl_module.*/ s/^#LoadModule/LoadModule/" /etc/httpd/conf.d/httpd.conf||  { writeJSONResponceErr "result=>4020" "message=>Cannot enable SSL module!"; return 4020; };
...
function _disableSSL(){
        ...
        sed -i "/^LoadModule.*ssl_module.*/ s/LoadModule/#LoadModule/" /etc/httpd/conf.d/httpd.conf ||  { writeJSONResponceErr "result=>4021" "message=>Cannot disable SSL module!"; return 4021; };
        service httpd restart >> /dev/null;
}
